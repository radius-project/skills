...
1. Select one runnable deployment profile. Treat explicit user or scenario requirements for Radius types, workload roles/count, native configuration keys, secret bindings, provider profile, protocol values, and connection names as acceptance criteria. Verify that the pinned source supports that profile; do not silently replace it with an easier default or optional backend. When the request leaves the profile open, model the application's primary feature through a compatible first-class Radius type before adding optional operational dependencies; do not replace a supported typed model, storage, or messaging resource with an external developer-supplied credential merely because one example manifest does so. For a model gateway or proxy, the primary feature is one configured end-to-end model route, not its optional management database or ingress.
...
- **Infer only when unspecified:** Without an explicit profile, prefer a source-supported profile that exercises the application's primary feature through compatible first-class Radius types. Use complete documented manifests to prove workload behavior, but do not let an optional database, monitoring service, or external API-key example displace the primary typed dependency. If multiple materially different primary-feature profiles remain valid, ask the user rather than choosing arbitrarily.
- **Model gateways require a model:** A runnable model gateway/proxy profile must expose at least one source-supported model alias backed by `Radius.AI/models` when that type and the target Recipe are compatible. If they are incompatible, fail closed instead of returning a database-only gateway. A database that stores gateway configuration, keys, or usage does not satisfy the inference feature and is not part of the minimal profile unless the selected route requires it or the request asks for that persistence.
...
| `build.source` for containerImages | Repo git URL: root `git::https://github.com/<org>/<repo>.git//?ref=<sha-or-tag>`; subdirectory `git::https://github.com/<org>/<repo>.git//<subdir>?ref=<sha-or-tag>` |
...
- Building from a complete Dockerfile context: add a `Radius.Compute/containerImages` resource with `build.source` set to the root repo URL (`git::https://github.com/<org>/<repo>.git//?ref=<sha-or-tag>`) or subdirectory URL (`.git//<subdir>?ref=<sha-or-tag>`); the container references the built image via `<serviceName>Image.properties.imageReference` (no separate connection needed).
...
- [ ] Every model gateway or proxy consuming `Radius.AI/models` separately proves the application-facing alias, provider deployment identifier, provisioned model identifier, endpoint, API/protocol version, managed-secret path/key, and any provider prefix its adapter requires against the exact recipe. A connection is not used as a fallback for a secret output.
