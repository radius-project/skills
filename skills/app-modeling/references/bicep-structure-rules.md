...
- `tag` is optional in the schema, but omission is allowed only when the exact recipe's computed-tag path is proven usable. If that path evaluates an invalid null interpolation or has another broken default, set a Docker-valid immutable tag derived from the pinned source ref
