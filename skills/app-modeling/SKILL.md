...
7. Choose a source build only when the repository has a complete, practical build context. For a BuildKit Git context, inspect the Dockerfile for required Git metadata; `COPY .git` or build commands that invoke Git require schema-supported `build.args.BUILDKIT_CONTEXT_KEEP_GIT_DIR: '1'`. When Git metadata is required but that argument cannot be supplied, use a pinned published image. Map every runtime value using [connection-conventions.md](references/connection-conventions.md), [secrets-handling.md](references/secrets-handling.md), and [bicep-structure-rules.md](references/bicep-structure-rules.md).
