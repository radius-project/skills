...
- Building from a complete Dockerfile context: add a `Radius.Compute/containerImages` resource with `build.source` set to the repo git URL pinned to the exact modeled checkout (`git::https://github.com/<org>/<repo>.git//<subdir>?ref=<checked-out-sha-or-explicit-immutable-tag>`); do not reuse `main`, `edge`, or another mutable deployment ref from an existing file. The container references the built image via `<serviceName>Image.properties.imageReference` (no separate connection needed).
...
- [ ] A source build has a complete practical Dockerfile/context and uses the exact checked-out source commit or an explicit immutable tag; an existing application's mutable `main`/`edge` deploy ref is not source evidence and is never copied. Otherwise the published image is pinned. Generated builds are consumed through `.properties.imageReference`.
