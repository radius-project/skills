...
6. First create or update `.radius/bicepconfig.json` (see [bicepconfig.json](#bicepconfigjson)), using any `bicepconfig.json` currently applicable to `.radius/app.bicep` as input. Then resolve every emitted type and planned property read/write against the Radius extension that `.radius/bicepconfig.json` declares. Record the verbatim property path and schema proof in the ledger; for recipes, also prove output mappings, managed-secret keys, and the implementation's handling of every omitted optional input the model relies on. A schema-optional property may be omitted only when the exact target Recipe handles its absent/null value safely. Reject an absent path before generation instead of guessing an alias, convenience property, or wrapper. Use the matching `resource-types-contrib` schema revision and target Environment recipe/output contract; do not copy shapes from another version. If a mutable local extension disagrees with that exact deployment contract, never edit the model to satisfy the stale extension: preserve the deployment-required shape, replace the alias only with a verified compatible immutable reference, or report the unresolved mismatch and stop before returning, committing, or pushing.
7. Choose a source build only when the repository has a complete, practical build context. Pin `build.source` to the exact checked-out source commit (or an explicit immutable tag supplied for that source); never copy a mutable branch such as `main` or `edge` from an existing application definition, manifest, or example. Otherwise use a pinned published image. Map every runtime value using [connection-conventions.md](references/connection-conventions.md), [secrets-handling.md](references/secrets-handling.md), and [bicep-structure-rules.md](references/bicep-structure-rules.md).
...
| `build.source` for containerImages | Repo git URL pinned to the modeled checkout: `git::https://github.com/<org>/<repo>.git//<subdir>?ref=<checked-out-sha-or-explicit-immutable-tag>` (`//<subdir>` only when the Dockerfile isn't at the repo root); never inherit a mutable branch from an existing deployment file |
...
First inspect the target repository's `bicepconfig.json`: the `radius` extension alias is the local compile-time contract. Resolve schemas from the `resource-types-contrib` revision that produced that artifact and from the Environment's registered type definition and Recipe. A mutable artifact such as `radius:latest`, a recipe tagged `latest`, or a branch ref can drift; warn about that uncertainty and do not mix its property shapes with a different revision. A clean compile against a mutable extension that disagrees with the target Environment is not validation and must not override the deployment contract.
...
Read the matching schema file for property names, types, sensitivity, read-only outputs, and API versions. The configured extension and type registered in the target Environment must agree. Stop and report a version mismatch rather than choosing one contract or guessing. In particular, never remove a schema-declared managed-secret binding or replace it with generic connection projection merely because an incompatible mutable extension exposes the secret as a direct property.
...
Default to the `radius:latest` tag only when no exact target Environment schema/Recipe contract is available. Pin a verified compatible immutable reference when a specific version is provided by an existing `bicepconfig.json`, the user, or the target Environment. Never retain or copy `radius:latest` after proving that its schema differs from the target deployment contract; if no compatible reference can be resolved, report the mismatch instead of weakening `app.bicep` to make the mutable artifact compile.
...
- Building from a complete Dockerfile context: add a `Radius.Compute/containerImages` resource with `build.source` set to the repo git URL pinned to the exact modeled checkout (`git::https://github.com/<org>/<repo>.git//<subdir>?ref=<checked-out-sha-or-explicit-immutable-tag>`); do not reuse `main`, `edge`, or another mutable deployment ref from an existing file. Inspect the exact target Recipe before omitting optional `tag`; when its absent/null path is not safe, set `tag` to a Docker-compatible immutable value derived from the source commit. The container references the built image via `<serviceName>Image.properties.imageReference` (no separate connection needed).
...
- Add `Radius.Compute/routes` only when the request, a deployment manifest, or source documentation explicitly requires a public/external URL. An HTTP listener, REST handlers, or a health endpoint alone is not external-ingress evidence.
...
- Never rely on generic connection projection for a sensitive output when the target schema/Recipe declares managed-secret metadata. Bind the app-native key with the exact `secretKeyRef` even if an incompatible mutable extension advertises that secret as a direct read-only property.
...
- [ ] Every planned resource property read/write has its verbatim path in the ledger and exists in the exact configured schema/API version. Every Recipe-generated output has a verified output mapping; every managed-secret reference has the declared secret-name path and key; every omitted optional input is proven safe in the exact Recipe implementation. An absent path blocks generation rather than being replaced by a guessed property, alias, or wrapper.
- [ ] Exactly one `Radius.Core/applications@2025-08-01-preview`, and one `extension radius` (no per-namespace or per-type extensions).
- [ ] The file compiles with an extension compatible with the target Environment schema/Recipe contract; every `Radius.*` type is on the allow-list and matches that version's schema and API version. Unknown type/property warnings are resolved, not ignored. A clean compile against a conflicting mutable extension is a version-drift failure, not permission to alter deployment-required properties or secret bindings.
...
- [ ] Every route is backed by explicit public/external-ingress evidence. An HTTP workload is not given a route merely because it exposes handlers or a health endpoint.
...
- [ ] A source build has a complete practical Dockerfile/context and uses the exact checked-out source commit or an explicit immutable tag; an existing application's mutable `main`/`edge` deploy ref is not source evidence and is never copied. `containerImages.tag` is omitted only when the exact target Recipe is proven null-safe; otherwise it is a Docker-compatible immutable value derived from the source revision. Otherwise the published image is pinned. Generated builds are consumed through `.properties.imageReference`.
