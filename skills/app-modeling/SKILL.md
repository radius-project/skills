...
4. Extract each workload's runtime contract: image/build context and platform support, entrypoint and arguments, listener and ports, required environment/configuration, secrets, writable paths and ownership, dependencies, wire protocols, and feature-critical configuration.
...
6. First create or update `.radius/bicepconfig.json` (see [bicepconfig.json](#bicepconfigjson)), using any `bicepconfig.json` currently applicable to `.radius/app.bicep` as input. Then resolve every emitted type and planned property read/write against the Radius extension that `.radius/bicepconfig.json` declares. Record the verbatim property path and schema proof in the ledger; for recipe outputs, also prove the output mapping and any managed-secret key. Reject an absent path before generation instead of guessing an alias, convenience property, or wrapper. Use the matching `resource-types-contrib` schema revision and target Environment recipe/output contract; do not copy shapes from another version. When the target Environment or user supplies an exact schema revision, that contract wins over a drifting mutable public extension: do not rewrite the model to the stale shape or spend the task searching registry tags. Validate with the target extension when available; otherwise write the target-correct model and report the public-extension compile blocker.
7. Choose a source build only when the repository has a complete, practical build context. Otherwise use a pinned published image. Resolve the exact container-image recipe's default platforms and override `build.platforms` when the Dockerfile cannot build every default target. Map every runtime value using [connection-conventions.md](references/connection-conventions.md), [secrets-handling.md](references/secrets-handling.md), and [bicep-structure-rules.md](references/bicep-structure-rules.md). Treat provider connection strings as opaque until their exact format is proved; a value named `connectionString` is not automatically an application URL, password, or generic connection projection.
8. Generate the Bicep using [naming-conventions.md](references/naming-conventions.md), then compile it with the exact configured extension. Treat every Bicep warning or error as a validation failure. Do not dismiss `use-secure-value-for-secure-inputs` for literal configuration stored in `Radius.Security/secrets`; deliver nonsecret generated configuration through a verified writable runtime path instead. Use an existing lightweight config linter when practical, but do not build the entire application solely to lint generated configuration when the pinned source already proves its syntax and fields.
...
- Compare the Dockerfile with the exact `containerImages` recipe's default platforms. If the recipe defaults to multiple targets but the Dockerfile lacks the required cross-build contract, set `build.platforms` to one target supported by the deployment profile instead of relying on the incompatible default.
...
- A schema-declared managed secret is not part of generic connection projection. Bind it from the exact managed-secret name/key even if an older or mutable extension exposes a similarly named direct property.
- For an abstract type backed by a concrete provider, the provider recipe determines the wire protocol, endpoint transformation, authentication, and secret format. Do not configure a RabbitMQ-native adapter merely because the type is named `rabbitMQ`, and do not pass a provider connection string to a URL field unless the source explicitly accepts that exact format.
...
- [ ] The file compiles with the target repository's exact configured extension with zero warnings and zero errors; every `Radius.*` type is on the allow-list and matches that version's schema and API version. `use-secure-value-for-secure-inputs` is a failure, including when caused by putting literal nonsecret configuration in `Radius.Security/secrets`.
- [ ] `param environment string` is declared; add a `@secure() param` for each developer-supplied secret.
- [ ] Every required executable role is modeled, including co-scheduled producer/consumer or proxy/backend roles. Roles in one requested verification pipeline are co-scheduled unless the source or profile requires independent deployment. Each role's image/build, entrypoint/arguments, listener, exposed ports, config artifacts, writable storage/ownership, and lifecycle are correct. `containerPort` matches the process; it does not configure the listener.
...
- [ ] A source build has a complete practical Dockerfile/context and uses an immutable ref; otherwise the published image is pinned. Its effective `build.platforms` are compatible with the Dockerfile and target runtime rather than an unchecked recipe default. Generated builds are consumed through `.properties.imageReference`.
...
- [ ] Every dependency has a complete client tuple: subresource name, endpoint/FQDN transformation, port, protocol/version, TLS mode, auth mechanism/identity, secret source and format, and final client syntax. Provider connection strings are parsed or consumed only according to their proved format. Provider modules, SKUs, regions, and firewall configuration remain outside `app.bicep`.
