...
4. Extract each workload's runtime contract: image/build context, entrypoint and arguments, listener and ports, required environment/configuration, secrets, writable storage, dependencies, wire protocols, and feature-critical configuration. Inspect CLI flags and structured fields as well as environment variables. Before constructing an aggregate credential-bearing URL/config, prefer source-native decomposed host, port, database, username, password, and TLS inputs when they let the application safely assemble the final client value.
...
7. Choose a source build only when the repository has a complete, practical build context. For a BuildKit Git context, inspect the Dockerfile for required Git metadata; `COPY .git` or build commands that invoke Git require schema-supported `build.args.BUILDKIT_CONTEXT_KEEP_GIT_DIR: '1'`. When Git metadata is required but that argument cannot be supplied, use a pinned published image. Resolve the exact recipe's default target platforms, require them to include the known runtime architecture, and prove the Dockerfile can build every one; when either check fails, set `build.platforms` to a supported runtime platform or stop if the target architecture is unspecified. Map every runtime value using [connection-conventions.md](references/connection-conventions.md), [secrets-handling.md](references/secrets-handling.md), and [bicep-structure-rules.md](references/bicep-structure-rules.md).
...
- An authored `Radius.Security/secrets.data.value` is not a runtime composition mechanism. Do not use Bicep interpolation there to manufacture a credential-bearing URL or connection string, regardless of whether the other parts come from outputs, parameters, variables, or literals. If the application accepts only one credential-bearing value, emit it only from an exact recipe-managed secret or a verified runtime path that can safely encode and compose the parts; otherwise stop and report the contract gap.
- Never satisfy an encoding requirement by assuming a `string` password is URL-safe or by recommending that callers choose restricted characters. Unless the source/schema explicitly constrains the character set, the generated model must handle arbitrary valid input.
...
- [ ] A BuildKit Git source preserves required Git metadata. If the Dockerfile copies `.git` or the build invokes Git for required metadata, the exact containerImages schema supports and receives `BUILDKIT_CONTEXT_KEEP_GIT_DIR: '1'`; source build is rejected only when that required metadata cannot be preserved.
- [ ] Every containerImages target platform is intentional. Recipe defaults are resolved, include the known runtime architecture, and are all supported by the Dockerfile; otherwise `build.platforms` is constrained to a supported runtime architecture. An unknown runtime architecture blocks a source build that cannot satisfy the recipe defaults.
...
- [ ] No authored secret value uses Bicep interpolation to construct an aggregate credential-bearing URL/config. Such a value is either an exact managed-secret output or is encoded and composed by a verified runtime mechanism; if neither path exists, generation stops instead of returning a compile-only model.
- [ ] Before composing an aggregate credential-bearing value, source-native decomposed flags/fields are inspected and preferred when they safely handle encoding. No model assumes a general `string` credential is URL-safe.
