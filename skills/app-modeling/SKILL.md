...
7. Choose a source build only when the repository has a complete, practical build context. Resolve the selected `containerImages` recipe's default platforms, inspect whether the Dockerfile is actually cross-build capable, and set `build.platforms` explicitly when its defaults would execute unsupported target-architecture binaries. Otherwise use a pinned published image. Map every runtime value using [connection-conventions.md](references/connection-conventions.md), [secrets-handling.md](references/secrets-handling.md), and [bicep-structure-rules.md](references/bicep-structure-rules.md).
...
- [ ] A source build has a complete practical Dockerfile/context and uses an immutable ref; otherwise the published image is pinned. Its requested/default platforms match both the Dockerfile's proven build behavior and the target runtime. A recipe defaulting to multiple platforms is overridden with an explicit supported platform list when the Dockerfile executes target binaries without cross-build support. Generated builds are consumed through `.properties.imageReference`.
