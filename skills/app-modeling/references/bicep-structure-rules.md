...
- Resolve the exact recipe default for `build.platforms`; omission is a platform choice, not a neutral default. The selected platforms must include the known runtime architecture, and a multi-platform default requires Dockerfile cross-compilation support for every target. Otherwise set a supported runtime platform explicitly (for example `['linux/amd64']`); if the target architecture is unknown, report the build-profile gap instead of emitting an image that may fail to build or run
