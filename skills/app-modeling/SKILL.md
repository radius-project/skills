...
8. Generate the Bicep using [naming-conventions.md](references/naming-conventions.md), then compile it by running `az bicep build` with the exact configured extension. Treat **any** Bicep diagnostic (warning or error) as a validation failure. A `use-secure-value-for-secure-inputs` warning means a property marked sensitive in the schema received a plain (non-`@secure()`) value — fix it by declaring a `@secure() param` and using it for that property. Surface the full diagnostic output to the user and do not proceed until the build is warning-free and error-free.
...
- [ ] The file compiles with `az bicep build` using the target repository's exact configured extension with zero warnings and zero errors. Any Bicep diagnostic is a validation failure: surface the full diagnostic output to the user and correct the Bicep before proceeding. A `use-secure-value-for-secure-inputs` warning means a sensitive schema property received a plain (non-`@secure()`) value — fix it by declaring a `@secure() param`. Every `Radius.*` type is on the allow-list and matches that version's schema and API version.
